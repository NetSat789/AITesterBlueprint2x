...
CSV_PATH = os.path.join(os.path.dirname(__file__), "vwo_login_test_cases.csv")

mcp = FastMCP("VWO Test Cases MCP Server")
...
@mcp.tool()
def list_all_test_cases(limit: int = 50) -> list[dict[str, str]]:
    """List all test cases with an optional limit (default 50, max 500)."""
    rows = load_test_cases()
...
def get_summary_stats() -> dict[str, Any]:
    """Get summary statistics of the test case repository (counts by priority, module, etc.)."""
    rows = load_test_cases()
    total = len(rows)
